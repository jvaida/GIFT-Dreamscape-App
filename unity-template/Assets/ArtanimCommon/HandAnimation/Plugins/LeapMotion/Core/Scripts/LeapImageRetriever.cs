    ...
    private void OnDestroy() {
      ...
      if (controller != null) {
		...
		private void LateUpdate()
		{
			...
			//Bart: Find the most recent image and use that
			//Different than the commented code above from the Leap API this
			//allows for services to be restarted and devices to be reconnected where
			//counts reset. The code above would no longer refresh in that case
			while (_imageQueue.TryPeek(out potentialImage))
			{
				if (usedImageId < potentialImage.SequenceId)
				{
					_currentImage = potentialImage;
					usedImageId = potentialImage.SequenceId;
				}
				_imageQueue.TryDequeue();
			}
		}
